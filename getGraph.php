    ...
    <head>
        ...
        <script>
            ...
            var data = { values:[
                ...
                $semGpa=getSemGpa(1,$user);
                if($semGpa!=0)
                echo '{ X: "Sem1", Y:'.$semGpa.' },';

                $semGpa=getSemGpa(2,$user_session);
                if($semGpa!=0)
                echo '{ X: "Sem2", Y:'.$semGpa.' },';

                $semGpa=getSemGpa(3,$user);
                if($semGpa!=0)
                echo '{ X: "Sem3", Y:'.$semGpa.' },';

               $semGpa=getSemGpa(4,$user);
                if($semGpa!=0)
                echo '{ X: "Sem4", Y:'.$semGpa.' },';

                $semGpa=getSemGpa(5,$user);
                if($semGpa!=0)
                echo '{ X: "Sem5", Y:'.$semGpa.' },';

               $semGpa=getSemGpa(6,$user);
                if($semGpa!=0)
                echo '{ X: "Sem6", Y:'.$semGpa.' },';

                $semGpa=getSemGpa(7,$user);
                if($semGpa!=0)
                echo '{ X: "Sem7", Y:'.$semGpa.' },';

               $semGpa=getSemGpa(8,$user);
                if($semGpa!=0)
